function sendData(SelectedCountry, SelectedCategory) {
  ...
  let url =
    "selected/" + callCountry(SelectedCountry) + callCategory(SelectedCategory);
  if (callCountry(SelectedCountry) === "bad") return; // Don't Do any thing if no country selected
...
function appendNews(response) {
  // the response is the object  of news
  // creating elements
  var flexbox = document.getElementById("flexbox");
  removeChildren(flexbox);
...
function appendCountries(response) {
  // the response is the object of countries
